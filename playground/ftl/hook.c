...
int overwrite_mem( mem * base_offsets, int target_location, void * data, int size) {
  long unsigned int * addr = base_offsets->executable_addr - base_offsets->offset;
  // ugly pointer math :(
  addr = (long unsigned int *)((long unsigned int)addr + target_location); 

  printf("[+] Overwriting memory at %p, size: %d\n", addr, size);
  if (mprotect((void *)0x400000, 0x42a000, PROT_READ | PROT_WRITE | PROT_EXEC) == -1) {
...
__attribute__ ((constructor)) int hook() {
  ...
  long unsigned int target_offset = 0x596920 - BASE_OFFSET;
  printf("[+] Target offset: %p\n", (void *)target_offset);

  if (target_location == -1) { return 1; }
  if (search_mem(SELF_PID, "FTL", mem_offsets) != 0) { return 1; }
  overwrite_mem(mem_offsets, target_offset, (char *)buffer, 8);

  /*Goal 3b: automatically find executable memory boundaries for memprotect, protect then unprotect*/
  /*Goal 3c: instructions for running each goal*/
